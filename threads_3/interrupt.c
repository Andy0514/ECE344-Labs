...
register_interrupt_handler(int verbose)
{
	...
	if (sigaction(SIG_TYPE, &action, NULL)) { // returns 0 (evaluates to false) on success
...
interrupts_set(int enabled)
{
	...
	set_signal(&mask);
	if (enabled) {
		// NOTE: sigprocmask is atomic, so there will not be inconsistencies 
		// between setting the signal and getting the old value into omask
...
interrupt_handler(int sig, siginfo_t * sip, void *contextVP)
{
	 ...
	 * this signal should be blocked because the signal that triggered
	 * the handler (SIG_TYPE) is automatically blocked. We don't want
	 * recursive interrupts to happen */
